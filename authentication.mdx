...
|--------|----------|------------|
| **Trial Token** | Trying Xpoz with no sign-up | Simplest |
...
<Tabs>
  <Tab title="Trial Token">
    ## Trial token (no sign-up)

    Want to try Xpoz before creating an account? Mint a free **trial token** — no sign-in required. It is valid for 5 days.

    ### Step 1: Mint a trial token

    ```bash
    curl -X POST https://api.xpoz.ai/api/trial/token
    ```

    The response contains a token that starts with `TRIAL`:
    ...
    Pass the `TRIAL...` token exactly like an access key — as the SDK `apiKey`, the `XPOZ_API_KEY` env var, or a bearer token.
    ...
    <Note>
      Trial tokens are intentionally limited: read-only data tools only, up to 5 cached results per call (no pagination or CSV export), and no live crawling. [Get your own access key](https://xpoz.ai/get-token) for full results, pagination, CSV export, and live data.
    </Note>
  </Tab>
