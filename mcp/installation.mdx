...
<Tip>
  No account yet? Mint a free trial token (valid 5 days) and use it in place of `YOUR_API_KEY` below:
  ...
  Trial tokens grant read-only tools with up to 5 cached results per call. See [Authentication](/authentication) for the full limits.
</Tip>
