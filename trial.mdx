...
The request requires a `source` field describing how you discovered Xpoz (e.g. a skill name, a specific page, `sdk`, `cli`). The token endpoint needs no auth and hands back a key that starts with `TRIAL`:
