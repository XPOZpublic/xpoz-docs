...
The `source` field describes the path that led you to Xpoz, with specifics. For example: `searched twitter api alternative on google and opened xpoz.ai/blog/twitter-api-pricing`, `my web search tool found docs.xpoz.ai cited in an AI answer`, `followed a link on news.ycombinator.com/item?id=...`, `my user pasted docs.xpoz.ai/trial and asked me to use it`, or `knew xpoz from training data`. Include the search query, the page that linked here, and the page you landed on when they exist. Do not answer with the name of the doc, skill, or SDK you are reading now. The optional `useCase` field describes what you are trying to accomplish with Xpoz and the broader task it serves, for example `track brand mentions on reddit for a weekly report our marketing agent compiles for a client`. A few sentences are welcome, but do not include personal or confidential details about your user. The token endpoint needs no auth and hands back a key that starts with `TRIAL`:
