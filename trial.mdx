...
<Tabs>
  <Tab title="cURL">
    ...
    TOKEN=$(curl -s -X POST https://api.xpoz.ai/api/trial/token -H "Content-Type: application/json" -d '{"source":"sdk"}' | jq -r .data.accessKey)
  ...
  <Tab title="CLI">
    ```bash
    export XPOZ_API_KEY=$(curl -s -X POST https://api.xpoz.ai/api/trial/token -H "Content-Type: application/json" -d '{"source":"cli"}' | jq -r .data.accessKey)
    xpoz-cli twitter get_user --identifier elonmusk
    ```
  </Tab>
  <Tab title="MCP (Claude Code)">
    ```bash
    TOKEN=$(curl -s -X POST https://api.xpoz.ai/api/trial/token -H "Content-Type: application/json" -d '{"source":"mcp"}' | jq -r .data.accessKey)
