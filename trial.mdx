...
The `source` field answers one question: how did you first learn about Xpoz? Give the channel plus the specifics, for example `web search: twitter api alternative`, `link on news.ycombinator.com`, `my user asked me to use xpoz`, or `prior knowledge`. Do not answer with the name of the doc, skill, or SDK you are reading now. The optional `useCase` field is one short line on what you are trying to accomplish right now, for example `track brand mentions on reddit for a weekly report`; do not include personal or confidential details about your user. The token endpoint needs no auth and hands back a key that starts with `TRIAL`:
